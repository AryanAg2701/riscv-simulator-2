...
do // this is just to run the entire search multiple runs for consistent stats reporting
{
	 ...
	 */
cudaEvent_t ssort1, esort1, ssort, esort;
float time_ms = 0;
// 	cudaEventCreate(&ssort);
// 	cudaEventCreate(&esort);
// 	cudaStreamSynchronize(streamKernels);

// // record start
// cudaEventRecord(ssort, streamKernels);
	compute_BestLSets_par_sort_msort<<<numQueries, numThreads_K3,0, streamKernels >>>(d_neighbors,
														...
														d_nextIter);
// cudaEventRecord(esort, streamKernels);
// cudaEventSynchronize(esort);

// // compute elapsed time in ms
// cudaEventElapsedTime(&time_ms, ssort, esort);
...
// // kernel_dummy<<<numQueries, numThreads_K3,0,streamKernels>>>();
...
// // compute elapsed time in ms
		 ...
		 * The kernel merges, for every query, two arrays each of whose sizes are upperbounded by L, so we do not require more than 2*L threads per query.
	// 	 */	
	// cudaEventCreate(&ssort1);
	// cudaEventCreate(&esort1);
	// cudaStreamSynchronize(streamKernels);
	// cudaEventRecord(ssort1, streamKernels);
		compute_BestLSets_par_merge<<<numQueries, numThreads_K3_merge,0, streamKernels >>>(d_neighbors,
										d_numNeighbors_query,
										d_neighborsDist_query,
										d_BestLSets,
										...
										d_BestLSets_count,
		 								...
		 								d_FPSetCoordsList_Counts,
		 								d_numQueries);
// 										cudaEventRecord(esort1, streamKernels);
// cudaEventSynchronize(esort1);
// cudaEventElapsedTime(&time_ms, ssort1, esort1);
...
__global__ void  compute_BestLSets_par_sort_msort(unsigned* d_neighbors,
    ...
    *d_nextIter = false;
	...
	__shared__ unsigned shm_neighbors_aux[64];
	__shared__ float shm_neighborsDist_query_aux[64];
	...
	if(tid < numNeighbors){
		shm_neighborsDist_query_aux[tid] = d_neighborsDist_query[offset + tid];
		shm_neighbors_aux[tid] = d_neighbors[offset + tid];
	}
	else{
		shm_neighborsDist_query_aux[tid] = FLT_MAX;
		shm_neighbors_aux[tid] = UINT_MAX;
	}
	__syncthreads();
    ...
    if (ij > tid && ij < 64)
    {
        float a = shm_neighborsDist_query_aux[tid];
        float b = shm_neighborsDist_query_aux[ij];
        if ((a > b) == ((tid & 2) == 0))
        {
            shm_neighborsDist_query_aux[tid] = b;
            shm_neighborsDist_query_aux[ij] = a;
			unsigned c = shm_neighbors_aux[tid];
			shm_neighbors_aux[tid] = shm_neighbors_aux[ij];
			shm_neighbors_aux[ij] = c;
        }
    }
    ...
    if(tid < numNeighbors)   
	{
		d_neighbors[offset + tid] = shm_neighbors_aux[tid];
		d_neighborsDist_query[offset + tid] = shm_neighborsDist_query_aux[tid];
	}
	...
	if(numNeighbors > 0){	// If the number of neighbors after filteration is zero then no sense of merging
        ...
        if(iter==1){	// If this is the first call to compute_BestLSets_par_merge by this query then initialize d_BestLSets, d_BestLSetsDist...
                ...
                for(unsigned ii=tid; ii < nbrsBound; ii += blockDim.x) {
                        ...
                        d_BestLSetsDist[queryID*L + ii] =   d_neighborsDist_query[offset + ii];
			...
			/*perform parallel merge */
                for(int i=tid; i < nbrsBound; i += blockDim.x) {
                        shm_neighborsDist_query[i] = __ldg(&d_neighborsDist_query[offset + i]);
                }
                for(int i=tid; i < Best_L_Set_size; i += blockDim.x) {
                        shm_currBestLSetsDist[i] = __ldg(&d_BestLSetsDist[L*queryID+i]);
                }
                __syncthreads();
                if(tid < nbrsBound) {
